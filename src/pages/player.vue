  ...
  <div :key="id" class="container relative">
    <div class="video-box">
      ...
      <template v-else>
        <xg-player
          ref="player"
          :active="is_player_active && isPageActive"
          :src="play_url"
          :poster="data.detail?.cover_thumb_url"
          :preview-config="{
      ...
      <nuxt-icon
        name="arrow-left"
        size="0.6rem"
        class="btn-back cursor-pointer text-4xl text-white"
        @click="$router.back"
      ></nuxt-icon>
    </div>
    ...
    <div class="scroll-container relative">
      <scroll-list :loading="loading || loading_comments" :pullup="fetch_comments" :is-empty="isEmpty" :is-end="isEnd">
        <div class="dx-list">
          <div class="info-top">
            <user-item
              :key="data.detail?.user?.uid"
              :is-follow="data.detail?.user?.is_attention"
              :item="data.detail?.user"
            ></user-item>
            <p class="info-title mb-1 line-clamp-2 font-medium">{{ data.detail?.title }}</p>
            <div class="flex flex-wrap gap-1">
              <dx-tag
                v-for="(item, index) in data.detail?.tags_list"
                :key="index"
                v-link="`/tag?&_type=video&title=${item}&has_sort=1&tag=${item}`"
                :text="item"
                :type="1"
              />
            </div>
          ...
          <template v-if="data.topic_info">
            ...
            <scroll-x-view v-if="collect.list?.length">
              <div class="collect-list">
                <video-card
                  v-for="(item, index) in collect.list"
                  :key="index"
                  class="inline-collect"
                  :item="item"
                  :list="collect.list"
                  :index="index"
          ...
          <div v-if="res_recommend.recommend?.length" class="info-recommend">
            ...
            <div class="grid grid-cols-2 gap-1">
              <video-card
                v-for="(item, index) in res_recommend.recommend"
                :key="index"
                :list="res_recommend.recommend"
                :index="index"
                lines
                replace
                show-type
    ...
    <!-- 合集 -->
    <video-collect-popup
      v-model:show="hejiLayer"
      :list="collect.list"
      :title="data.topic_info?.title"
    ></video-collect-popup>
