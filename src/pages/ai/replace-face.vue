...
<template>
  <div v-if="key" :key="key" class="container">
    <scroll-list :loading="spining" :pullup="route.query.id ? _getComments : undefined" :is-end="isEnd"
      ...
      <div class="dx-grid-2">
        <div>
          <div v-if="is_custom_tpl" class="h-[230px] rounded-sm bg-mgray">
            <van-field class="my-upload h-full w-full" name="uploader" label-align="top">
              <template #input>
                <van-uploader v-model="tpl_images" :max-size="2 * 1024 * 1024" reupload :preview-full-image="false"
                  :after-read="upload_tpl_image" accept="image/*" :max-count="1" @delete="delete_custom_tpl">
        ...
        <div>
          <div class="h-[230px] rounded-sm bg-mgray">
            <van-field class="my-upload h-full w-full" name="uploader" label-align="top">
              <template #input>
                <van-uploader v-model="images" reupload :preview-full-image="false" :after-read="upload_image"
                  accept="image/*" :max-count="1" @delete="delete_target_image">
      ...
      <div class="dx-grid-1 mt-3">
        ...
        <dx-button v-else class="text-white" :round="false" color="linear-gradient(to right, #FFC300, #FFB052)"
          @click="submit(0)">
        ...
        <div class="grid grid-cols-1 gap-1">
          <ai-comment-item v-for="item in listData" :key="item.id" :item="item" @comment="onBeforeComment">
            <template #right="{ comment }">
              <dx-btn-like :id="comment.id" class="ml-auto" align="row" :like="comment.is_like === 1"
                api="api/ai/like_comment" :likes="comment.like_num" icon="good-job-o" active-icon="good-job" :params="{
                  type: 1,
                  action_type: 0
                }" :use-toast="false" active-color="#f7931d" />
            </template>
          </ai-comment-item>
        </div>
      </div>
    ...
    <!-- 底部评论 -->
    <dx-comment-input v-if="route.query.id" ref="commentRef" v-model="commentValue" v-model:reply="reply"
      style="--dx-base2-color: #898989; --dx-base10-color: #e2e2e2" class="whitespace-nowrap bg-[#F3F3F3]"
      api="api/ai/commentnew" :params="{ comment_id: reply.id, material_id: route.query.id }"
      @success="refresh_comments">
      <template #left>
        <dx-btn-like :id="route.query.id" :like="data.detail?.is_like === 1" :likes="data.detail?.like"
          api="api/ai/like_comment" :use-toast="false" size="0.5rem" icon="like-o" active-icon="like"
          active-color="#FF0000" :params="{
            type: 0,
            action_type: 0
          }">
          <template #default>
        ...
        <dx-btn-like :id="route.query.id" class="mx-1" :like="data.detail?.is_favorite === 1" :likes="data.detail?.like"
          api="api/ai/like_comment" :use-toast="false" size="0.5rem" icon="star-o" active-icon="star"
          active-color="#FF0000" :params="{
            type: 0,
            action_type: 1
          }">
    ...
    <!-- 购买提示弹框 -->
    <dx-buy-popup v-model:paying="paying" v-model:show="show_buy"
      :api="is_custom_tpl ? 'api/ai/customize_face' : 'api/ai/change_face'" :params="options"
      :show-success-alert="false" :coins="Number(is_custom_tpl ? data.ai_ht_coins : cache[CACHE_KEY.AI_FACE].coins)"
      :free-num="data.free_num" :pay-type="options.type ? 2 : 1" success-text="上传成功,等待处理"
      :success-handler="res => res.status === 1" type="face" @pay:success="onPay" />
  </div>
</template>
