  ...
  <xg-tk-player ref="player" :video-info="{
    ...
    <template #default="{ seeking }">
      <transition name="van-fade">
        <div v-show="!seeking">
          <div class="layer-container absolute bottom-2 left-1 z-[999] w-[176px]">
            ...
            <div v-show="data.user?.nickname" class="mb-0.5 flex items-center text-xl">
              @{{ data.user?.nickname }}
            ...
            <ul v-if="tags_list.length" class="tag-list truncate text-sm">
              <nuxt-link v-for="tag in tags_list?.slice(0, 3)" :key="tag"
                :to="`/tag?&_type=discover&title=${tag}&has_sort=1&tag=${tag}`" class="tag-item text-white"
          ...
          <div class="layer-container absolute bottom-2 right-1 z-20">
            <div class="grid w-[46px] grid-cols-1 gap-1.5 text-mini">
              <dx-avatar v-if="item?.user" v-show="expand" class="mb-1.5" :size="1.2" :img="item?.user?.avatar_url"
                :uid="item?.user?.uid" :aff="item?.user?.aff" :creator="false">
                <template v-if="item.user.uid !== user.uid" #badge>
                  <dx-btn-like :id="item.user?.uid" id-key="to_uid" :use-toast="false"
                    :like="item.user?.is_attention === 1" :show-count="false"
