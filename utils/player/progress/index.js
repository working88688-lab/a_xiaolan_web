...
export class CustomProgress extends Plugin {
  ...
  _mouseUpHandler = (e, data) => {
    const { pos, playerConfig, player, offsetDuration } = this

    const { previewTime } = playerConfig
    /**
     *   uaa 新增
     */
    if (previewTime && previewTime < data.currentTime) {
      const percent = previewTime / offsetDuration
      this.updatePercent(percent)
      this.updateTime(previewTime)
      player.currentTime = previewTime
      this._state.now = previewTime
      this._state.direc = previewTime > data.currentTime ? 0 : 1
  ...
  updateWidth(currentTime, seekTime, percent, type) {
     ...
     *   uaa 新增
     */
    if (playerConfig.previewTime && playerConfig.previewTime < realTime) {
      return
    }
  ...
  computeTime(e, x) {
    ...
    const currentTime = Number.parseInt(percent * this.offsetDuration * 1000, 10) / 1000
  ...
  updatePercent(percent, notSeek) {
    ...
    const { miniprogressplugin } = this.player.plugins
    miniprogressplugin && miniprogressplugin.update({ played: percent * this.offsetDuration }, this.offsetDuration)
  }
  ...
  onTimeupdate(isEnded) {
    const { player, _state, offsetDuration, playerConfig } = this
    const { previewTime } = playerConfig
    /**
     *   uaa 新增
     */
    if (previewTime && previewTime < player.currentTime) {
      const percent = previewTime / offsetDuration
      this.updatePercent(percent)
      this.updateTime(previewTime)
      player.currentTime = previewTime
      this._state.now = previewTime
      this._state.direc = previewTime > player.currentTime ? 0 : 1
    ...
    if (_state.now > -1) {
      const abs = Number.parseInt(_state.now * 1000, 10) - Number.parseInt(player.currentTime * 1000, 10)
  ...
  beforePlayerInit() {
    this.i18n.MEDIA_ERR_DECODE = '资源加载失败，反馈给客服'
  }
