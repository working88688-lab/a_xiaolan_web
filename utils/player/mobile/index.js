import { Events, Plugin, Sniffer, STATES, Util } from 'xgplayer'
...
export class Mobile extends Plugin {
  ...
  afterCreate() {
    ...
    if (controls && controls.root) {
      controls.root.addEventListener('touchmove', this.onRootTouchMove, true)
      controls.root.addEventListener('touchend', this.onRootTouchEnd, true)
      controls.root.addEventListener('touchcancel', this.onRootTouchEnd, true)
    }
  ...
  changeAction(action) {
    ...
    player.root.setAttribute('data-xg-action', action)
    const startPlugin = player.plugins.start
    startPlugin && startPlugin.recover()
    player.emit('seeking-action', action)
  }
  ...
  onTouchStart = e => {
    ...
    if (touche && !config.disableGesture && this.duration > 0 && !player.ended) {
      ...
      const _x = Number.parseInt(touche.pageX - pos.left, 10)
      const _y = Number.parseInt(touche.pageY - pos.top, 10)
    ...
    if (Math.abs(x - pos.x) > miniMoveStep || Math.abs(y - pos.y) > miniMoveStep) {
      ...
      if (scope === -1) {
        ...
        if (scope === 0) {
          ...
          !pos.time && (pos.time = Number.parseInt(player.currentTime * 1000, 10) + this.timeOffset * 1000)
        }
        pos.scope = scope
      }
  ...
  updateVolume(percent) {
    ...
    percent = Number.parseInt(percent * 100, 10)
    pos.volume += percent
  ...
  destroy() {
    ...
    if (controls && controls.root) {
      controls.root.removeEventListener('touchmove', this.onRootTouchMove, true)
      controls.root.removeEventListener('touchend', this.onRootTouchEnd, true)
      controls.root.removeEventListener('touchcancel', this.onRootTouchEnd, true)
    }
  }
